...
def main():
    ...
    for i in range(0, len(STOCKS), 5):
        ...
        data_tab2_list = [historical_market_cap_api(x, START_DATE, END_DATE, 100000) for x in batch]
        data_tab1_map = {data["symbol"]: data for data in data_tab1_list}
        for data_tab2 in data_tab2_list:
            tab2.append({"id": tab2_id, "ticker": data_tab2[0]["symbol"], "data": [{"date": x["date"], "market_cap": x["marketCap"]} for x in data_tab2]})
            symbol = data_tab2[0]["symbol"]
            tab1.append({"ticker": symbol, "market_cap": data_tab1_map[symbol]["marketCap"], "pointer": tab2_id})
            tab2_id += 1
